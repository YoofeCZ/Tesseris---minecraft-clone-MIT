...
// Fragment shader ROSTLIN ve vzdalenem terenu.
//
// PROTI far.frag ma NAVIC ALFA TEST. Trava, kapradi a kytky jsou zkrizene plochy s dirami
// v texture — bez zahozeni pruhlednych fragmentu by kolem kazdeho stebla byl neprusvitny
// obdelnik.
//
// PROC NENI SOUCASTI far.frag. Discard zakazuje grafice early-Z: dokud shader nedobehne,
// U rostlin se to zaplatit musi, u terenu by to byla cista ztrata — a teren je drtiva
// vetsina plochy dlazdice.
//
// PROC NENI SOUCASTI chunk.frag (vyrezovy pruchod chunku). Ten nema blizky rez podle
// pc.chunkOffset.w, ktery u LOD dlazdic zahazuje cast lezici uvnitr dosahu chunku.
// Bez nej by trava z LOD stala mezi skutecnou travou chunku.
//
...
void main()
{
    ...
    // STOCHASTICKE RIDNUTI S DALKOU.
    //
    // Trs travy je par pixelu siroky a stoji na pravidelne mrizce sloupcu. Kdyz se jich do
    // jednoho pixelu vejde nekolik, nevznikne trava, ale sum — a protoze mrizka je
    // pravidelna, sklada se z nej moire, ktere se pri chuzi vlni. Presne to bylo videt
    // na obzoru.
    //
    // Misto ostreho konce se proto stebla PLYNULE PRORIDUJI: kazde dostane vlastni prah
    // podle sve pozice a mizi tim driv, cim vys ho nahoda posadila. Prubeh je nahodny
    // v prostoru, ale STALY V CASE — hash zavisi jen na svetove pozici, takze se trs pri
    // pohybu kamery neobjevuje a nemizi.
    //
    // Zbytek dojede mlha: FogStart je 288, takze u 700 bloku uz je krajina z vetsiny
    // preplacnuta do barvy oblohy a chybejici stebla nikdo nepozna.
    float keep = 1.0 - smoothstep(340.0, 700.0, vDistance);
    ...
    // ALFA TEST. Prah MUSI sedet na chunk.frag, jinak by stebla na hranici pasma zmenila
    // tloustku. (Prvni verze tu mela 0,5 s poznamkou, ze to je "tyz jako u chunku"; nebyla
    // to pravda.)
    if (texel.a < 0.02)
    {
        discard;
    ...
    // NOCNI ZTLUMENI AZ ZA PODVODNIM ROZPTYLEM — viz far.frag, tataz oprava.
